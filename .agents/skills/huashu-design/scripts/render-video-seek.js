...
if (!HTML_FILE || HTML_FILE.startsWith('--')) {
  ...
  console.error(
    'Example: NODE_PATH=$(npm root -g) node render-video-seek.js my-animation.html --fps=60'
  )
  process.exit(1)
}
...
console.log(
  `  size: ${WIDTH}x${HEIGHT} · ${FPS}fps · duration: ${DURATION}s · frames: ${TOTAL_FRAMES} · workers: ${CONCURRENCY}`
)
  ...
  await page.evaluate(
    (count) =>
      new Promise((resolve) => {
        ...
        const step = () => {
          i++
          i >= count ? resolve() : requestAnimationFrame(step)
        }
...
;(async () => {
  ...
  try {
    await Promise.all(
      buckets.map((b) => (b.length ? renderFrames(context, url, b) : Promise.resolve()))
    )
  } catch (e) {
  ...
  const ffmpeg = spawnSync(
    ...
    [
      ...
      String(FPS),
      '-i',
      path.join(TMP_DIR, 'frame-%06d.png'),
      '-c:v',
      'libx264',
      '-pix_fmt',
      'yuv420p',
