    ...
    <div className="p-8 bg-primary-bg/60 backdrop-blur-md border border-accent-cyan/20 rounded-3xl shadow-2xl">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-separate border-spacing-y-3">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={4} className="text-center py-10 animate-pulse text-accent-cyan">Cargando base de datos...</td></tr>
            ) : users.map((user) => (
              <motion.tr 
                key={user.id}
              ...
              >
                <td className="px-4 py-4 rounded-l-2xl border-l border-y border-white/5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-white font-semibold text-sm">{user.fullName || 'Sin nombre'}</p>
                ...
                <td className="px-4 py-4 border-y border-white/5">
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                    user.role === 'admin' ? 'bg-accent-magenta/20 text-accent-magenta border border-accent-magenta/30' : 'bg-accent-cyan/20 text-accent-cyan border border-accent-cyan/30'
                  }`}>
                    {user.role}
                  </span>
                </td>
                ...
                <td className="px-4 py-4 rounded-r-2xl border-r border-y border-white/5 text-right">
                  <button className="p-2 text-primary-secondary hover:text-accent-magenta transition-colors" title="Eliminar">
                    <Trash2 size={18} />
                  </button>
                </td>
              </motion.tr>
