...
const Badge: React.FC<{ text: string }> = ({ text }) => (
  <motion.div
    ...
    className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full border border-cyan-500/30 bg-cyan-950/40 backdrop-blur-md mb-8 group/badge shadow-sm"
  >
    <span className="relative flex h-2 w-2">
      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
      <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-400" />
    </span>
    <span className="text-[10px] md:text-xs font-bold uppercase tracking-[0.25em] text-cyan-400 group-hover/badge:text-white transition-colors">
      {text}
    </span>
  </motion.div>
)
    ...
    >
      ...
      <span
        className={`relative inline-block ${
          isPunchline
            ? 'text-transparent bg-clip-text bg-gradient-to-r from-sky-400 via-cyan-300 to-fuchsia-400 font-black drop-shadow-[0_2px_20px_rgba(14,165,233,0.45)]'
      ...
      >
        ...
        {isPunchline && (
          <motion.span
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 bg-gradient-to-r from-transparent via-white/40 to-transparent bg-clip-text text-transparent"
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center px-3 sm:px-4 text-center max-w-5xl mx-auto w-full">
        ...
        {/* Hero Banner Card — Dark Cyber Glass with Cyan Glow */}
        <motion.div
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1], delay: 1.7 }}
          className="inline-flex flex-col items-center gap-1.5 px-4 py-3.5 sm:px-7 sm:py-4 rounded-xl sm:rounded-2xl bg-[#090a16]/95 border border-cyan-500/30 backdrop-blur-xl mb-5 sm:mb-6 max-w-2xl shadow-[0_10px_35px_rgba(6,182,212,0.18)] hover:border-cyan-500/60 transition-all w-full sm:w-auto"
        >
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-cyan-400 shrink-0" />
            <p className="text-xs sm:text-sm md:text-base text-slate-100 font-bold">
              {t('hero.titulo_2')}
            </p>
          </div>
        ...
        <motion.p
          ...
          className="text-sm sm:text-lg md:text-xl text-slate-200 leading-relaxed max-w-3xl mx-auto mb-6 sm:mb-7 font-semibold px-2"
        >
          {t('hero.descripcion')}
        </motion.p>
        ...
        >
          ...
          <MagneticButton href="#contact">
            <span className="w-full sm:w-auto text-center px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl border border-cyan-500/30 text-white font-extrabold bg-[#0e101c] backdrop-blur-md hover:bg-[#141728] hover:border-cyan-500/50 transition-all text-xs sm:text-sm tracking-wider uppercase block shadow-md hover:shadow-lg">
              {t('hero.cta_proyecto')}
            </span>
          </MagneticButton>
        </motion.div>
