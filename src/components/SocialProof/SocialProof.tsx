...
const SocialProof: React.FC = () => {
  ...
  const renderTrack = (trackKey: string, isAriaHidden = false) => (
    <div
      ...
      className="flex shrink-0 items-center gap-3.5 pr-3.5 animate-marquee will-change-transform transform-gpu group-hover:[animation-play-state:paused]"
    >
      {CLIENTS.map((client) => {
        ...
        return (
          <div
            key={`${trackKey}-${client.idx}`}
            className={`group/card relative flex flex-col items-center justify-between p-4 rounded-xl border transition-[transform,background-color,border-color,box-shadow] duration-200 ease-out cursor-pointer overflow-hidden shrink-0 w-40 sm:w-44 select-none ${
              isCyan
                ? 'border-border/60 dark:border-white/10 bg-card/50 dark:bg-slate-900/50 hover:border-cyan-500/60 hover:bg-cyan-500/[0.08] hover:shadow-[0_0_22px_rgba(14,165,233,0.22)] hover:-translate-y-1'
                : 'border-border/60 dark:border-white/10 bg-card/50 dark:bg-slate-900/50 hover:border-amber-500/60 hover:bg-amber-500/[0.08] hover:shadow-[0_0_22px_rgba(245,158,11,0.22)] hover:-translate-y-1'
            }`}
          >
            {/* Radial Glow Layer on Hover */}
            <div
              className={`absolute -inset-full rounded-xl opacity-0 group-hover/card:opacity-100 transition-opacity duration-300 pointer-events-none blur-lg ${
                isCyan
                  ? 'bg-gradient-to-tr from-cyan-500/20 via-sky-400/10 to-transparent'
                  : 'bg-gradient-to-tr from-amber-500/20 via-orange-400/10 to-transparent'
              }`}
            />
            ...
            {/* Icon Container */}
            <div
              className={`w-10 h-10 rounded-lg flex items-center justify-center border transition-[transform,background-color,border-color] duration-200 my-1 ${
                isCyan
                  ? 'border-border/50 dark:border-white/10 bg-background/80 dark:bg-white/[0.03] group-hover/card:border-cyan-500/40 group-hover/card:bg-cyan-500/15 group-hover/card:scale-105'
                  : 'border-border/50 dark:border-white/10 bg-background/80 dark:bg-white/[0.03] group-hover/card:border-amber-500/40 group-hover/card:bg-amber-500/15 group-hover/card:scale-105'
              }`}
            >
              <Icon
  ...
  return (
    <section className="py-10 relative overflow-hidden z-10 border-y border-border/30 bg-transparent">
      ...
      <div className="relative z-10">
        ...
        {/* Seamless Infinite Dual-Track Marquee */}
        <div className="group relative flex w-full overflow-hidden py-2 select-none">
          {renderTrack('track-primary', false)}
          {renderTrack('track-secondary', true)}
        </div>
      </div>
    </section>
  )
