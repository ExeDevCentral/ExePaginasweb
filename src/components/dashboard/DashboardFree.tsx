  ...
  return (
    <div className="relative">
      <motion.div
        ...
        className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-white/5 to-transparent p-8 backdrop-blur-xl"
      >
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-accent-cyan font-bold">Sin abono activo</p>
            <h1 className="text-3xl sm:text-4xl font-black text-white mt-1">Hola, {nombre}</h1>
            <p className="mt-2 text-primary-secondary max-w-lg">
              Elegí un plan para desbloquear tu panel operativo. Cada nivel tiene su propio dashboard con métricas y
              herramientas adaptadas a tu negocio.
            </p>
            <button
              type="button"
              onClick={onLogout}
              className="mt-4 text-xs font-bold text-white/40 hover:text-accent-magenta"
            >
              Cerrar sesión
            </button>
          </div>
      ...
      <p className="mt-10 mb-4 text-xs font-bold uppercase tracking-[0.3em] text-white/40 text-center">
        ...
        {PREVIEW_TIERS.map(({ tier, icon: Icon }, i) => {
          ...
          return (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className={`rounded-3xl border ${t.border} bg-[#08080c]/60 p-6 backdrop-blur-xl cursor-default`}
            >
              <div className={`w-12 h-12 rounded-xl border ${t.border} flex items-center justify-center mb-4 ${t.glow}`}>
              ...
              <ul className="mt-4 space-y-1.5">
                {t.perks.slice(0, 3).map((p) => (
                  <li key={p} className="text-xs text-white/60">
                    · {p}
                  </li>
                ))}
              </ul>
              <p className="mt-4 text-[10px] font-mono text-white/25">dashboard.{tier}.exe</p>
            </motion.div>
          )
        })}
      </div>
