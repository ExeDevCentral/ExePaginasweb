      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            key="mob-drawer"
            className="lg:hidden fixed top-16 left-0 right-0 bottom-0 z-50 flex flex-col bg-[#07080f] dark:bg-[#07080f] border-t border-slate-800 dark:border-white/10 shadow-2xl overflow-hidden"
            style={{
          ...
          >
            {/* Cabecera del Drawer */}
            <div className="flex items-center justify-between px-5 py-3 border-b border-white/10 bg-[#0c0d18]">
              ...
              <button
                ...
                className="flex items-center gap-1 text-xs font-bold text-white bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-lg border border-white/15 cursor-pointer active:scale-95 transition-all"
              >
            ...
            <nav
              data-lenis-prevent
              className="flex flex-col px-5 py-4 gap-2 flex-1 overflow-y-auto bg-[#07080f]"
            >
              {/* Enlace Inicio */}
              <motion.a
                href="#home"
                className={`p-3.5 rounded-xl border transition-all flex items-center justify-between ${
                  activeId === 'home'
                    ...
                    : 'bg-[#0e101c] border-white/10 text-slate-100 hover:border-cyan-500/40 hover:bg-[#141728]'
                }`}
                onClick={(e) => scrollToSection(e, 'home')}
                whileTap={{ scale: 0.98 }}
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-extrabold text-white">{t('nav.inicio')}</span>
                </div>
                {activeId === 'home' && (
                  <span className="px-2 py-0.5 rounded-md bg-cyan-500/30 text-cyan-300 text-[10px] font-bold">
                    Activo
                  </span>
                )}
              </motion.a>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-extrabold text-white">{t('nav.productos')}</span>
                </div>
                {activeId === 'products' && (
                  <span className="px-2 py-0.5 rounded-md bg-cyan-500/30 text-cyan-300 text-[10px] font-bold">
                    Activo
              ...
              <motion.a
                href="/cotizador"
                className="p-3.5 rounded-xl border bg-[#0e101c] border-emerald-500/40 hover:border-emerald-400 transition-all flex items-center justify-between"
                onClick={() => setIsMenuOpen(false)}
                whileTap={{ scale: 0.98 }}
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="px-1.5 py-0.2 rounded bg-emerald-500/25 text-emerald-300 text-[9px] font-mono font-bold">
                      PRO
                    </span>
                  </div>
                </div>
              ...
              <motion.a
                href="/tienda"
                className="p-3.5 rounded-xl border bg-[#0e101c] border-cyan-500/40 hover:border-cyan-400 transition-all flex items-center justify-between"
                onClick={(e) => {
                  e.preventDefault()
                  setIsMenuOpen(false)
                  navigate('/tienda')
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-extrabold text-white">{t('nav.contacto')}</span>
                </div>
                {activeId === 'contact' && (
                  <span className="px-2 py-0.5 rounded-md bg-cyan-500/30 text-cyan-300 text-[10px] font-bold">
                    Activo
              ...
              <a
                ...
                onClick={() => setIsMenuOpen(false)}
                className="mt-1 p-3.5 rounded-xl border bg-emerald-950/60 border-emerald-500/40 hover:bg-emerald-900/60 transition-all flex items-center justify-between cursor-pointer"
              >
                <div className="flex items-center gap-3">
              ...
              {isLoggedIn && (
                <div className="mt-2 p-2.5 rounded-xl bg-cyan-950/40 border border-cyan-500/30 flex flex-col gap-1.5">
                  <motion.button
                    onClick={() => {
                      navigate('/dashboard')
                      setIsMenuOpen(false)
                    }}
                    className="w-full py-2 px-3 rounded-lg text-xs font-bold text-cyan-300 text-left flex items-center justify-between bg-cyan-500/10 hover:bg-cyan-500/20"
                  >
                    <div className="flex items-center gap-2">
                  ...
                  <motion.button
                    onClick={handleLogout}
                    className="w-full py-1.5 px-3 rounded-lg text-xs font-bold text-rose-400 text-left flex items-center gap-2 hover:bg-rose-500/10"
                  >
                    <LogOut size={14} />
              ...
              <div className="mt-auto pt-4 pb-4 border-t border-white/10 flex flex-col gap-2.5">
                <button
                  type="button"
                  onClick={goToClientArea}
