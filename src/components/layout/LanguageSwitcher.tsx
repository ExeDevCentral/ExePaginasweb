...
export default function LanguageSwitcher({ className = '' }: Readonly<LanguageSwitcherProps>) {
  ...
  return (
    <div className={`relative group ${className}`}>
      <button
        type="button"
        className="h-8 px-2 flex items-center justify-center gap-1.5 rounded-lg text-slate-300 hover:text-white hover:bg-white/[0.08] transition-all text-xs font-semibold cursor-pointer select-none"
      ...
      <div className="absolute right-0 top-full mt-2 bg-[#0c0d14]/98 border border-white/15 rounded-xl shadow-2xl shadow-black/80 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 min-w-[155px] p-1.5 backdrop-blur-2xl">
        <div className="px-2.5 py-1 mb-1 border-b border-white/10 text-[10px] font-sans font-semibold uppercase tracking-wider text-slate-400">
          Idioma / Language
        </div>
        {LANGUAGES.map((lang) => {
          const isSelected =
