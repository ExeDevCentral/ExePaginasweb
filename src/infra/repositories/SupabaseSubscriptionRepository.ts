...
export class SupabaseSubscriptionRepository {
  async getByClienteId(clienteId: string): Promise<Suscripcion[]> {
    ...
    const { data: planes } = slugs.length > 0
      ? await supabase.from('planes').select('slug, nombre, precio, caracteristicas').in('slug', slugs)
      : { data: [] }
    ...
    return subs.map(s => ({
      ...s,
      plan_id: s.plan_slug,
      plan: planMap.get(s.plan_slug) || { slug: s.plan_slug, nombre: s.plan_slug, precio: null, caracteristicas: null },
