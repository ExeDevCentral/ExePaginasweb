...
export const PREMIUM_TOKENS = {
  // Page Background Container (Adapts: #f8fafc en Light, #030308 en Dark)
  bgMain:
  ...
  glowWrap:
    'relative p-[1px] rounded-[32px] bg-gradient-to-b from-cyan-500/20 via-purple-500/10 to-pink-500/20 dark:from-cyan-500/40 dark:via-purple-500/20 dark:to-pink-500/40 shadow-lg dark:shadow-[0_0_60px_-15px_rgba(14,165,233,0.3)]',

  // SaaS Navigation Tabs Bar
  tabsBar:
    'mb-8 p-1.5 bg-slate-100/90 dark:bg-[#090a12]/90 border border-slate-200 dark:border-white/15 rounded-2xl backdrop-blur-2xl flex items-center gap-1.5 overflow-x-auto touch-pan-x shadow-md dark:shadow-lg scrollbar-none transition-colors duration-300',
  ...
  // Inactive Tab Button
  inactiveTab:
    'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200/60 dark:hover:bg-white/5 font-semibold transition-all',
  ...
  // SSL Badge
  sslBadge:
    'hidden sm:flex items-center gap-2 text-[11px] font-mono text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50 backdrop-blur-xl px-4 py-1.5 rounded-full border border-emerald-300 dark:border-emerald-500/30 shadow-sm dark:shadow-[0_0_15px_rgba(16,185,129,0.15)]',

  // Input Field Container
  inputWrap:
    'relative rounded-2xl border transition-all duration-200 bg-white dark:bg-slate-950/80 border-slate-200 dark:border-white/15 hover:border-slate-300 dark:hover:border-white/25 focus-within:border-cyan-500 dark:focus-within:border-cyan-400 focus-within:ring-2 focus-within:ring-cyan-500/20 text-slate-900 dark:text-white',

  // Text Color Tokens for Maximum Contrast
  textPrimary: 'text-slate-900 dark:text-white font-extrabold',
  textSecondary: 'text-slate-600 dark:text-slate-300 font-medium',
  textMuted: 'text-slate-500 dark:text-slate-400 font-normal',
} as const
