...
async function analyzeAccessibility() {
  ...
  if (fs.existsSync('src')) {
    ...
    console.log(
      `  • Files with alt text: ${filesWithAlt}/${totalFiles} (${Math.round((filesWithAlt / totalFiles) * 100)}%)`
    )
    console.log(
      `  • Files with ARIA attributes: ${filesWithAria}/${totalFiles} (${Math.round((filesWithAria / totalFiles) * 100)}%)`
    )
    console.log(
      `  • Files with semantic HTML: ${filesWithSemantic}/${totalFiles} (${Math.round((filesWithSemantic / totalFiles) * 100)}%)`
    )
